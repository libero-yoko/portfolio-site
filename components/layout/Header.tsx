...
const styles = {
  'expandable-menu':'absolute z-0 flex flex-col h-screen w-screen gap-8 text-4xl text-white bg-stone-400 items-center justify-center -translate-y-full transition ease-in-out duration-300 [&>li:hover]:text-black md:hidden',
}
export default function Header(): React.ReactElement {
  ...
  const navMenuLink = pages.map(page => <li key={page.id!}><a href={`/${page.slug}`}>{page.title}</a></li>)
  return (
